...
void HistogramImpl::Clear() {
  min_ = (double) bucketMapper.LastValue();
  max_ = 0;
  num_ = 0;
  sum_ = 0;
  sum_squares_ = 0;
...
void HistogramImpl::Add(uint64_t value) {
  ...
  buckets_[index] += 1;
  min_ = std::min(min_, (double) value);
  max_ = std::max(max_, (double) value);
  num_++;
  sum_ += value;
...
double HistogramImpl::Percentile(double p) const {
  ...
  double sum = 0;
  for (unsigned int b = 0; b < bucketMapper.BucketCount(); b++) {
    sum += (double) buckets_[b];
    if (sum >= threshold) {
      // Scale linearly within this bucket
      double left_point = (b == 0) ? 0 : (double) bucketMapper.BucketLimit(b - 1);
      double right_point = (double) bucketMapper.BucketLimit(b);
      double left_sum = sum - (double) buckets_[b];
...
std::string HistogramImpl::ToString() const {
  std::string r;
  char buf[200];
  _snprintf_s(buf, sizeof(buf), 200,
